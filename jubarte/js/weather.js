...
const WeatherWidget = {
  ...
  async fetchWeather(retryCount = 0) {
    try {
      ...
      if (retryCount < 2) {
        setTimeout(() => this.fetchWeather(retryCount + 1), 10000);
      }
      this.displayError();
    }
  },
  ...
  displayError() {
    ...
    const lang = window.i18n ? window.i18n.getLanguage() : 'pt';
    const errorText = { pt: 'Indisponível', en: 'Unavailable', he: 'לא זמין' };

    if (tempEl) tempEl.textContent = '--°C';
    if (iconEl) iconEl.textContent = '🌡️';
    if (descEl) descEl.textContent = errorText[lang] || errorText.pt;
  },
